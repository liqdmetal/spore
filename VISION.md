# Spore multi-chain messenger

*Vision / architecture note. This project was built and verified live on DERO, then
...
## 1. What this project actually is (don't lose this)

This project is NOT a chain. It is a **transport + coordination layer** that rides on
...
Everything else — the ephemeral-ECDH crypto, the secure E2E
...
| Chain | Encrypted payload | Wallet/signer | Fit |
|---|---|---|---|
| **DERO** | ✅ native (point-to-point, ring sig) | ✅ `--rpc-server` | **live, mainnet-verified** |
| **EVM-compatible** | ⚠️ calldata/events public → secure envelope | wallet/signer | **live-verified** (local anvil); `MyceliumMailbox.sol` for busy chains |
| **Solana** | ⚠️ inbox PDA public → secure envelope | signer keypair | **live on mainnet** (BPF program `GbNWrv…BraAs`) |
...
**The rule that makes it portable:** where a chain has no native per-recipient
encrypted message field (EVM, Solana, XMR), privacy comes from **the secure
envelope** — X25519 ECDH + HKDF-SHA256 + XChaCha20-Poly1305
(`internal/secure`, `kind 0xE0`) layered on whatever the chain carries. The
chain is identity + a carrier; the secure envelope is the secrecy. On-chain records (calldata,
...
- **XMR tx = a knock, not content.** The backend posts a dust transfer whose
  8-byte payment id carries a short signal (a reference/knock), never
...
- **Monero gives identity + a payment rail**, not an encrypted-message
...
## 4. The interconnection model — the relay fabric
...
own P2P). This project slots in at the transport seam:

```
app (whisper / rooms / long-body)
   -> core (ECDH crypto, secure envelope, rendezvous, compost, UI)  [chain-agnostic]
      -> signer seam:  DERO | EVM | Solana | Monero                  [per-chain]
      -> transport seam: node P2P | substrate mesh (Waku/Iroh/libp2p)
           -> relay nodes = the shared fabric
```

**Relay nodes as the fabric:** an always-on relay node connects
instances across chains — a DERO user and an EVM user both reach their
...
**Interchain (DERO wallet ↔ EVM wallet) — the honest model:** direct
point-to-point encryption across chains is impossible (different key
...
1. Sender encrypts the body to a key the recipient can derive by proving control
   ...
   problem, NOT a messenger problem).
2. The pointer/notification crosses chains via the relay mesh.
...
Because identity = your wallet address on whatever chain you're on,
a "relay" operator can advertise **one address per supported
...
6. ⏳ **Relay fabric interconnection** — relay node (encrypted pointer
...
- **Content stays nobody-but-us.** The relay fabric relays ciphertext only; it
  must never hold a key. If a substrate can read the body, it's not this project.
