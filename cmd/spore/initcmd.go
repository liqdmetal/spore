...
func initcmd(args []string) {
	...
	fmt.Println("  1. self-host or use the hosted mailbox; the device keeps the keys:")
	fmt.Printf("       spore prekeybatch push -in %s -mailbox https://HOST/u/USER -token TOKEN\n", batchPath)
	fmt.Println("  2. add a contact from THEIR identity card, so you can message them by name:")
	fmt.Println("       spore msg mail add -addr THEIR_ADDR -nick alice -pinned THEIR_PINNED_SIG")
	fmt.Println("  3. receive locally (foreground; Ctrl-C stops):")
	fmt.Println("       spore msg recv-e2 -store https://HOST/u/USER -store-token TOKEN")
	fmt.Println("  4. send — by nickname (address + pinned sig come from your address book):")
	fmt.Println("       spore msg send-e2 -to alice -bundle-url https://THEIR-HOST/u/alice/prekey")
	fmt.Println("     (plaintext via -msg-file or stdin — never argv)")
	fmt.Println("  5. optional arrival alert: see docs/NOTIFICATIONS.md")
	fmt.Println()
