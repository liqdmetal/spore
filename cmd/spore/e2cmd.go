...
//		sporepeer://host:port -> SporePeerStore: P2P — the sender's node holds and
//	                        serves bodies (spore-peer JSON subset), the
//	                        receiver fetches by CID; neither relies on any
//	                        third-party storage
//
// The nostr:// form was the first no-operator posture; sporepeer:// is the
// no-SERVERS endgame proper (roadmap: "serverless bodies over spore-peer"):
// only the two endpoints ever hold bytes, and the sender's hold composts on
// its own disk at the burn deadline.
//
...
// for the nostr scheme and ignored otherwise.
//
// sporepeer:// serves on -store-serve (bind addr, optional): without it the
// store can still fetch from the peer and hold bodies for the local side, but
// a contact cannot pull FROM this node — senders who want their bodies served
// (the whole point for send-e2) pass e.g. -store-serve 0.0.0.0:8099.
//
// relayBase, when set, wraps the HTTP store so WRITES take an anonymous relay
// hop (internal/relay.RelayStore): the mailbox then sees the relay's IP, and the
// sender never needs the mailbox's per-user token — which, on a hosted
...
// sendE2Opts holds the send-e2 flag values (extracted so tests can assert on
// the exact flag surface — audit H1 regression guards).
type sendE2Opts struct {
	to, identity, bundle, bundleURL, bundleToken, pinned *string
	amount, msgFile                                      *string
	ttl                                                  *time.Duration
	escrow, escrowHash, escrowRecipient                  *string
	escrowExpiry                                         *uint64
}

// newSendE2Flagset registers the send-e2 flag surface. NOTE (audit H1/H2):
// deliberately NO -key/-peer-pub flags — ratcheted frames carry no outer
// envelope; do not re-add per-frame envelope wrapping here.
func newSendE2Flagset() (*flag.FlagSet, *sendE2Opts) {
	fs := flag.NewFlagSet("msg send-e2", flag.ExitOnError)
	o := &sendE2Opts{}
	o.to = fs.String("to", "", "recipient chain address")
	o.identity = fs.String("identity", "", "file containing sender identity private key hex")
	o.bundle = fs.String("bundle", "", "recipient SPK bundle JSON file (mutually exclusive with -bundle-url)")
	o.bundleURL = fs.String("bundle-url", "", "fetch recipient SPK bundle from a mailbox GET /prekey URL (mutually exclusive with -bundle; discovery is a transport convenience only — -pinned-sig is still required and still verified)")
	o.bundleToken = fs.String("bundle-token", "", "bearer token for -bundle-url, if the mailbox requires auth")
	o.pinned = fs.String("pinned-sig", "", "recipient signing public key hex")
	o.amount = fs.String("amount", "", "pay-with-message: value to attach to the SAME tx as the pointer, in whole units with asset suffix (e.g. 5.5dero, 0.0001evm). DERO + EVM-calldata only today; the money and the message are atomic — both land or neither does")
	o.msgFile = fs.String("msg-file", "", "file containing plaintext (use '-' or omit for stdin; never pass plaintext as an argv flag — argv is visible to shell history, ps, and crash reports)")
	o.ttl = fs.Duration("ttl", 24*time.Hour, "frame retention")
	// Escrow flags: when set, the value rides an HTLC contract instead of direct transfer
	o.escrow = fs.String("escrow", "", "escrow mode: htlc (hash timelock). If set, -amount is locked in an HTLC contract and -escrow-hash, -escrow-recipient, -escrow-expiry become required")
	o.escrowHash = fs.String("escrow-hash", "", "32-byte hex preimage hash for HTLC escrow (SHA256(preimage))")
	o.escrowRecipient = fs.String("escrow-recipient", "", "DERO address that can claim the HTLC with the preimage")
	o.escrowExpiry = fs.Uint64("escrow-expiry", 0, "HTLC expiry block height (absolute); funds refundable to sender after this height")
	// Secure wire flags REMOVED (audit H1/H2): envelope-v2 wrapping on ratcheted
	// frames contradicted RATCHET.md §6 ("the outer 0xE1-style sig is REPLACED by
...
func msgSendE2(args []string) {
	fs, o := newSendE2Flagset()
	_ = fs.Parse(args)
	if err := loadConfigForFlags(fs); err != nil {
		check(err)
	}
	// Rebind to the historical local names so the body below is unchanged.
	to, identity, bundle, bundleURL, bundleToken, pinned := o.to, o.identity, o.bundle, o.bundleURL, o.bundleToken, o.pinned
	amount, msgFile, ttl := o.amount, o.msgFile, o.ttl
	escrow, escrowHash, escrowRecipient, escrowExpiry := o.escrow, o.escrowHash, o.escrowRecipient, o.escrowExpiry
...
func sendE2Core(fs *flag.FlagSet, to, identity, bundle, bundleURL, bundleToken, pinned, msgFile, amount string, ttl time.Duration, escrow, escrowHash, escrowRecipient string, escrowExpiry uint64) error {
	...
	// Secure wire (audit H1/H2): the envelope-v2 wrapper was removed from the
	// ratcheted path. The X3DH handshake + double ratchet authenticate frames;
	// the -pinned-sig bundle check is the trust anchor. See RATCHET.md §6.
...
type recvE2Opts struct {
	identity, spk, opk, opkPool       *string
	...
	outDir, ntfy                      *string
	notifyEmail, notifySMS            *string
	notifySMTPHost                    *string
	notifySMTPPort                    *int
	notifySMTPFrom, notifySMTPUser    *string
	notifyTwilioSID, notifyTwilioFrom *string
}

// newRecvE2Flagset registers the recv-e2 flag surface. NOTE (audit H1/H2):
// deliberately NO -key flag — ratcheted frames carry no outer envelope to
// unwrap; the -identity key is what the ratchet uses.
func newRecvE2Flagset() (*flag.FlagSet, *recvE2Opts) {
	fs := flag.NewFlagSet("msg recv-e2", flag.ExitOnError)
	o := &recvE2Opts{}
	o.identity = fs.String("identity", "", "our identity private key file")
	o.spk = fs.String("spk", "", "our signed-prekey private key file")
	o.opk = fs.String("opk", "", "legacy single one-time-prekey private key file")
	o.opkPool = fs.String("opk-pool", "", "endpoint-local persistent one-time-prekey pool JSON (preferred)")
	o.interval = fs.Duration("interval", 3*time.Second, "poll interval (active cadence)")
	...
	o.outDir = fs.String("out-dir", "", "write each received message body to a file in this dir (named <txid>.msg) instead of stdout — attachments/keep-a-copy mode")
	o.ntfy = fs.String("ntfy", "", "POST a metadata-only notification to an ntfy/webhook URL on each message (plaintext never leaves the client)")
	o.notifyEmail = fs.String("notify-email", "", "send a metadata-only email notification to this address (SMTP settings come from environment)")
	o.notifySMS = fs.String("notify-sms", "", "send a metadata-only SMS notification to this E.164 number (Twilio settings come from environment)")
	o.notifySMTPHost = fs.String("notify-smtp-host", "", "SMTP host for metadata-only email notifications")
	o.notifySMTPPort = fs.Int("notify-smtp-port", 587, "SMTP port for metadata-only email notifications")
	o.notifySMTPFrom = fs.String("notify-smtp-from", "", "SMTP From address for metadata-only email notifications")
	o.notifySMTPUser = fs.String("notify-smtp-user", "", "SMTP username (password from SPORE_NOTIFY_SMTP_PASSWORD)")
	o.notifyTwilioSID = fs.String("notify-twilio-sid", "", "Twilio account SID (auth token from SPORE_NOTIFY_TWILIO_AUTH_TOKEN)")
	o.notifyTwilioFrom = fs.String("notify-twilio-from", "", "Twilio sender number")
	// -maildb is registered by e2Common below (shared with send paths for
	// name resolution); read it after Parse rather than redeclaring it — a
