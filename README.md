...
|---|---|---|
| **Whisper** (no-relay unicast) | `compost whisper send/recv` | a short line rides a DERO tx payload, encrypted point-to-point; your node delivers it. No server. |
| Mailbox (Model A) | `compost daemon` + `send` | durable per-endpoint inbox; bodies off-chain, anchor on-chain |
| IRC rooms | `compost channel` + `chat` | public/private rooms, presence, TTL-bounded lines |
| Web chat | `compost web` | same rooms in the browser, optional TLS |
...
- **Compost**: bodies are TTL-evicted; keys are ephemeral and erased; the
...
`compost send` → min-postage anchor tx mined; `compost daemon` recv → body
...
BSD 3-Clause. Compost is clean-room Go; it imports no derohe source. derohe-rs
