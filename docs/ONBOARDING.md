...
One command — installs `spore` and the `spore-peer` transport for your
platform, SHA-256-verified against the release:

```bash
# Linux / macOS / WSL — installs into ~/.local/bin:
curl -fsSL https://raw.githubusercontent.com/liqdmetal/spore/main/scripts/install.sh | sh
```

```powershell
# Windows (PowerShell) — installs into ~\bin:
irm https://raw.githubusercontent.com/liqdmetal/spore/main/scripts/install.ps1 | iex
```

Prefer to read before piping to a shell? Fair:
`curl -fsSL -o install.sh https://raw.githubusercontent.com/liqdmetal/spore/main/scripts/install.sh && less install.sh && sh install.sh`.
...
| Platform | spore | spore-peer (transport) |
|---|---|---|
| Linux x86-64 | `spore-linux-amd64` | `spore-peer-linux-amd64` |
| Linux arm64 (RPi-class home nodes) | `spore-linux-arm64` | `spore-peer-linux-arm64` |
| macOS Apple Silicon | `spore-darwin-arm64` | `spore-peer-darwin-arm64` |
| macOS Intel | `spore-darwin-amd64` | `spore-peer-darwin-amd64` |
| Windows x86-64 | `spore-windows-amd64.exe` | `spore-peer-windows-amd64.exe` |
| Windows arm64 | `spore-windows-arm64.exe` | — (Go `spore` only; the transport's Rust cross-linker for this target isn't trustworthy yet — the installers skip it with a note, not an error) |

Every asset has a sibling `.sha256`, and every release ships a
`multiple.intoto.jsonl` SLSA provenance attestation (keyless, generated by
[slsa-github-generator](https://github.com/slsa-framework/slsa-github-generator)).
Verify a binary the way CI does:

```bash
slsa-verifier verify-artifact spore-linux-amd64 \
...
Build from source instead: `go install github.com/liqdmetal/spore/cmd/spore@latest`.

`spore-peer` is only needed when a postures stores bodies on a
`sporepeer://` endpoint **you run** — plain send/receive against someone's
mailbox or the nostr commons doesn't require it.
