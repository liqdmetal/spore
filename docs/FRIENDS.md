# Chat with a friend using compost — 5-minute setup

Both you and your friend each need **two things running**: your DERO wallet's
RPC server, and the `compost` binary. Everything else is a command.

## 1. Get the binaries

**compost** (download, no Go needed):
- Linux/macOS/Windows binaries: https://github.com/liqdmetal/compost/releases/tag/v0.1.0
- pick your OS, download `compost-<os>-<arch>`, `chmod +x` (linux/mac), put it on PATH.

**compost-peer** (only needed for LONG messages, not short whispers):
- Build from https://github.com/liqdmetal/compost-peer (`cargo build --release`)
...
must be online to transfer the body, and the sender must run `compost-peer
