...
| **F3** — **audit opened; drain-union shipped; jitter remaining** | N-relay redundancy + drain-union dedupe by CID, per-handle quotas + jitter, durable fabric index, `AUDIT-RELAYFABRIC.md` with the same hash-pinned remediation treatment. Landed so far: the audit (scope-vs-commits table; one HIGH fixed — immortal envelopes, spore-peer `78e69a4` `no-verify-hash`; two MEDIUMs fixed) and the **drain-union** (spore `3ec84c0`): one consumed-CID set across relays and restarts, marked on ingest success only, so M-relay redundancy costs ONE body fetch and a failed fetch stays eligible for a redundant copy. Remaining: R-N2 drain jitter, operator-facing fabric knobs | doc-refs CI over the new audit (live in `docs/refs.yml`) |
