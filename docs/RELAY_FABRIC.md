...
| **F3** — **audit opened; slice work remaining** | N-relay redundancy + drain-union dedupe by CID, per-handle quotas + jitter, durable fabric index, `AUDIT-RELAYFABRIC.md` with the same hash-pinned remediation treatment. The audit is landed: scope-vs-commits table, one HIGH fixed (immortal envelopes — the horizon-cap MUST was unenforced; spore-peer `78e69a4` `no-verify-hash`), two MEDIUMs fixed (sweep amplifier, expired-equals-absent), and the remaining F3 work tracked as R-N1 (drain-union dedupe) and R-N2 (drain jitter) | doc-refs CI over the new audit (live in `docs/refs.yml`) |
