...
| **F4** — **designed; F4a + F4b shipped** | transport adapters (Iroh/Waku sidecars) behind `FabricTransport`; cover traffic; multi-hop onion publish; CBOR method variants | designed in [`RELAY_FABRIC_F4.md`](RELAY_FABRIC_F4.md): the seam, Poisson cover traffic, the `fput2` onion envelope, the quantified timing analysis (jitter = hygiene, cover/onion = structure, volume is the floor), and the F4a–F4d slice plan. **F4a shipped**: the `FabricTransport` seam (TCP + in-memory), the rpc2/CBOR method family on the same cores as the JSON verbs, byte-exact `fabric_v1.rpc2_*` vectors, and the cross-binary CBOR roundtrip. **F4b shipped**: the Poisson cover scheduler (exponential waits, independent of the drain clock — distribution-shape-tested), per-pointer decoy bodies (fresh CID every cover), `spore fabric cover` mirroring the real send fan-out, the `-fabric-cover-rph` / `-fabric-cover-fold` knobs, and the drain-side decoy pre-filter (recipient cost: zero fetches). Build order is the addendum's — vectors first, audit on landing |
...
1. **Handle epochs — DECIDED (this decision is F1's wire-freeze baseline).**
   ...
   *inferable* (dual-publish is public). This is the same correlation the
   ...
   self-expiring). Both are specified as F4 publish-planner defaults in
   [`RELAY_FABRIC_F4.md`](RELAY_FABRIC_F4.md) §4.
